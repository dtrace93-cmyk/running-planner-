document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateLatestRunCards({
        distanceMiles,
        paceString,
        timeString,
        hrValue
    ...
    function renderTrendsStats(sessions, weekly) {
        ...
        const weatherGroups = sessions.reduce((acc, s) => {
            ...
            acc[key].count += 1;
            if (s.paceSeconds) {
                acc[key].paceTotal += s.paceSeconds;
                acc[key].paceCount += 1;
            }
            return acc;
        }, {});
    ...
    function computeHrTrendStats(sessions = []) {
        ...
        if (!hrSessions.length) {
            return {
                overall: null,
                halvesText: 'Not enough data yet',
    ...
    function computeTimeOfDayStats(sessions = []) {
        const normalized = (sessions || []).map(s => ({ ...s,
            timeOfDay: (s.timeOfDay || 'Unknown').toLowerCase()
        }));
    ...
    function generateInsights(sessions = [], summary = {
        weeks: []
    }, loadMetrics = {}) {
        const strengths = [];
        const improvements = [];
